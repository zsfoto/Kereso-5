...
/**
 * CategoriesCompanies Controller
 *
 * @property \App\Model\Table\CategoriesCompaniesTable $CategoriesCompanies
 */
class CategoriesCompaniesController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('CategoriesCompanies'));
		...
		$this->paginate['CategoriesCompanies']['page'] 	= $page;
		$this->paginate['CategoriesCompanies']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			$showSearchBar	 = true;
			$query = $this->CategoriesCompanies->find()
			...
			$query = $this->CategoriesCompanies->find()->contain(['Categories', 'Companies'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['CategoriesCompanies']['limit'] = $this->config['paginate_limit'];
			$categoriesCompanies = $this->paginate($query);
		...
		if(empty($categoriesCompanies->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('categoriesCompanies'));
    }

    /**
     * View method
     *
     * @param string|null $id Categories Company id.
     * @return \Cake\Http\Response|null|void Renders view
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('categoriesCompany') . ' ' . __('record'));
        $categoriesCompany = $this->CategoriesCompanies->get($id, contain: ['Categories', 'Companies']);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $categoriesCompany->name;
        $this->set(compact('categoriesCompany', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('categoriesCompany') . ' ' . __('record'));
        $categoriesCompany = $this->CategoriesCompanies->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $categoriesCompany = $this->CategoriesCompanies->patchEntity($categoriesCompany, $data);
			//debug($categoriesCompany);
			//die();
            if ($this->CategoriesCompanies->save($categoriesCompany)) {
                //$this->Flash->success(__('The categories company has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $categoriesCompany->id);

                //return $this->redirect(['action' => 'add']);
                return $this->redirect([
					'controller' => $this->controller,
					'action' => 'index',
					'#' => $categoriesCompany->id
				]);

            }
            //$this->Flash->error(__('The categories company could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $categories = $this->CategoriesCompanies->Categories->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $companies = $this->CategoriesCompanies->Companies->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('categoriesCompany', 'categories', 'companies'));
    }
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('categoriesCompany') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		
        $categoriesCompany = $this->CategoriesCompanies->get($id, contain: []);
        ...
        $companies = $this->CategoriesCompanies->Companies->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $categoriesCompany->name;
        $this->set(compact('categoriesCompany', 'categories', 'companies', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
        ...
        $categoriesCompany = $this->CategoriesCompanies->get($id);
        if ($this->CategoriesCompanies->delete($categoriesCompany)) {
			$this->session->delete('Layout.' . $this->controller . '.LastId');
            //$this->Flash->success(__('The categories company has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The categories company could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
