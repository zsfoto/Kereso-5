...
class EmailsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Emails'));
		...
		$this->paginate['Emails']['page'] 	= $page;
		$this->paginate['Emails']['order'] 	= [$sort => $direction];
		// ############################# /.SORT ORDER & PAGE ###############################
		...
		if($search !== ''){
			...
			$query = $this->Emails->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Emails']['limit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('email') . ' ' . __('record'));
        $email = $this->Emails->get($id, contain: []);
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $email->name;
        $this->set(compact('email', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('email') . ' ' . __('record'));
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Emails->save($email)) {
                //$this->Flash->success(__('The email has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $email->id);
            ...
            //$this->Flash->error(__('The email could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('email') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Emails->delete($email)) {
            ...
            //$this->Flash->success(__('The email has been deleted.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
        } else {
            //$this->Flash->error(__('The email could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
