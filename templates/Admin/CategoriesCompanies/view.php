 ...
 * @var \App\Model\Entity\CategoriesCompany $categoriesCompany
 */
						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('Categories Company') ?></h3>
									<?= __('The data marked with <span class="fw-bold text-danger">*</span> must be provided!') ?>
								</div>
								<div class="float-end ms-5">
									<?= $this->Html->link('<span class="btn btn-outline-secondary mt-1 me-1"><i class="fa fa-times"></i></span>',
										["controller" => "Categories Companies", "action" => "index", "_full" => true],
...
<?php } ?>
											<div class="row"><!-- 1. -->
												...
												<div class="col-sm-10 p-1 link">
													<?= $categoriesCompany->hasValue('category') ? $this->Html->link($categoriesCompany->category->name, ['controller' => 'Categories', 'action' => 'view', $categoriesCompany->category->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 1. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Company') ?>:</label>
												<div class="col-sm-10 p-1 link">
													<?= $categoriesCompany->hasValue('company') ? $this->Html->link($categoriesCompany->company->title, ['controller' => 'Companies', 'action' => 'view', $categoriesCompany->company->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 2. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Action') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= h($categoriesCompany->action) ?>
											...
											<div class="row"><!-- 3. -->
												...
												<div class="col-sm-10 p-1">
													<?= $this->Number->format($categoriesCompany->visible) ?><!-- 3.b -->
												</div>
											</div>
