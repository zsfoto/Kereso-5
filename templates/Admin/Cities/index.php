...
<?php } ?>
											...
											<th class="char zip"><?= $this->Paginator->sort('zip') ?></th><!-- H.1. -->
...
<?php } ?>
											...
											<td class="char zip" value="<?= $city->zip ?>"><?= h($city->zip) ?></td>
											<td class="string name" value="<?= $city->name ?>"><?= h($city->name) ?></td>
											<td class="string latitude" value="<?= $city->latitude ?>"><?= h($city->latitude) ?></td>
											<td class="string longitude" value="<?= $city->longitude ?>"><?= h($city->longitude) ?></td>
											<td class="char action" value="<?= $city->action ?>"><?= h($city->action) ?></td>
<?php if($config['show_pos']){ ?>
											<td class="number pos" value="<?= $city->pos ?>"><?= h($city->pos) ?></td>
<?php } ?>
<?php if($config['show_visible']){ ?>
											<td class="boolean visible" value="<?= $city->visible ?>"><?= h($city->visible) ?></td>
<?php } ?>
<?php if($config['show_counters']){ ?>
											<td class="number counter company-count" value="<?= $city->company_count ?>"><?= h($city->company_count) ?></td><?php } ?>
<?php if($config['show_created'] || $config['show_modified']){ ?>
											<td class="datetime">
<?php if($config['show_created']){ ?>
												<span class="fw-bold"><?= h($city->created) ?></span>
<?php } ?>
<?php if($config['show_created'] && $config['show_modified']){ ?>
												<br>
<?php } ?>
<?php if($config['show_modified']){ ?>
												<span class="fw-normal"><?= h($city->modified) ?></span>
<?php } ?>
											</td>
<?php } ?>
<?php if($config['show_button_view'] || $config['show_button_edit'] || $config['show_button_delete'] ){ ?>
