...
<?php } ?>
											<td class="string link company-id" value="<?= $person->company_id ?>"><?= $person->hasValue('company') ? $this->Html->link($person->company->title, ['controller' => 'Companies', 'action' => 'view', $person->company->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span></td>
											<td class="string name" value="<?= $person->name ?>"><?= h($person->name) ?></td>
											<td class="string slug" value="<?= $person->slug ?>"><?= h($person->slug) ?></td>
											<td class="string title" value="<?= $person->title ?>"><?= h($person->title) ?></td>
											...
											<td class="char action" value="<?= $person->action ?>"><?= h($person->action) ?></td>
<?php if($config['show_pos']){ ?>
